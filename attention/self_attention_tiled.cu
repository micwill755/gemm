...
extern "C" void self_attention_tiled_cuda(
    ...
    matmul_tiled_cuda(x, W_q, d_Q, seq_len, d_model, d_model);
    matmul_tiled_cuda(x, W_k, d_K, seq_len, d_model, d_model);
    matmul_tiled_cuda(x, W_v, d_V, seq_len, d_model, d_model);
    ...
    matmul_tiled_cuda(d_Q, d_K_T, d_scores, seq_len, d_model, seq_len);
    ...
    matmul_tiled_cuda(d_scores, d_V, d_context, seq_len, seq_len, d_model);
    
    // Step 6: Final output projection using tiled matmul
    matmul_tiled_cuda(d_context, W_o, output, seq_len, d_model, d_model);
...
extern "C" void self_attention_coalesced_b_cuda(
    float* x,           // Input [seq_len × d_model]
    float* W_q,         // Query weights [d_model × d_model]  
    float* W_k,         // Key weights [d_model × d_model]
    float* W_v,         // Value weights [d_model × d_model]
    ...
    // Step 1: Compute Q, K, V projections using coalesced_b matmul
    ...
    // Step 3: Compute attention scores using coalesced_b matmul
    matmul_coalesced_b_cuda(d_Q, d_K_T, d_scores, seq_len, d_model, seq_len);
    
    // Step 4: Apply tiled softmax to attention scores
    softmax_tiled_cuda(d_scores, seq_len, seq_len);
    
    // Step 5: Compute context using coalesced_b matmul
    matmul_coalesced_b_cuda(d_scores, d_V, d_context, seq_len, seq_len, d_model);
    
    // Step 6: Final output projection using coalesced_b matmul
